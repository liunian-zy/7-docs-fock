...
import { PINECONE_UPSERT_VECTOR_LIMIT, OPENAI_OUTPUT_DIMENSIONS, PINECONE_METRIC, PINECONE_POD_TYPE } from '@7-docs/shared';
...
export class Pinecone {
    ...
    async createIndex(name, environment) {
        if (!name)
            throw new Error('Please provide a name for the index');
    ...
    async upsertVectors({ namespace, vectors }) {
        const url = this.url;
        if (!url)
            throw new Error('Missing PINECONE_URL environment variable');
        let v = 0;
